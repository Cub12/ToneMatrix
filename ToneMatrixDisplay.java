...
public class ToneMatrixDisplay extends GCanvas implements MouseListener, MouseMotionListener, ComponentListener {
    ...
    private final boolean[][] matrix = new boolean[ToneMatrixConstants.size()][ToneMatrixConstants.size()];
    private final GRect[][] matrixRects = new GRect[ToneMatrixConstants.size()][ToneMatrixConstants.size()];
    private double xScale;
    private double yScale;
    private double xTrans;
    private double yTrans;
    private boolean isMarking;
    private final GRect highlightRect;
    ...
    private void redrawAll() {
        for (int row = 0; row < ToneMatrixConstants.size(); ++row) {
            for (int col = 0; col < ToneMatrixConstants.size(); ++col) {
                this.matrixRects[row][col].setBounds(this.xScale * (double) col + this.xTrans, this.yScale *
                        (double) row + this.yTrans, this.xScale, this.yScale);
        ...
        if (this.highlightedColumn == -1) {
            ...
            this.highlightRect.setBounds(this.xScale * (double) this.highlightedColumn + this.xTrans, this.yTrans,
                    this.xScale,this.yScale * (double) ToneMatrixConstants.size());
        }
    }
    ...
    public boolean[][] getMatrix() {
        ...
        for (int i = 0; i < ToneMatrixConstants.size(); ++i) {
            System.arraycopy(this.matrix[i], 0, result[ToneMatrixConstants.size() - 1 - i], 0,
    ...
    public void setMatrix(boolean[][] newMatrix) {
        if (newMatrix == null) {
            ...
            for (row = 0; row < ToneMatrixConstants.size(); ++row) {
                if (ToneMatrixConstants.size() >= 0)
                    System.arraycopy(newMatrix[row], 0, this.matrix[ToneMatrixConstants.size() - 1 - row],
                            0, ToneMatrixConstants.size());
            }
